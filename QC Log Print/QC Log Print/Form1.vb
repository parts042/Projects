...
Public Class Form1
    ...
    Const GPS_FW = "EGPS-1.2.103 06/26/2012"
    Const Ztrak_FW = "REV:22b | APP:132"
    ...
    Private Sub CheckArgs()
        ...
        If oXLSheet.UsedRange.Columns.Count = 66 Or oXLSheet.UsedRange.Columns.Count = 68 Or oXLSheet.UsedRange.Columns.Count = 73 Or oXLSheet.UsedRange.Columns.Count = 77 Or oXLSheet.UsedRange.Columns.Count = 2 Then ' Only necessary for GPS and 2020 QC files
            For i As Integer = 0 To Args.Count - 1
                Select Case LCase(Args(i))
                    Case "/navimport"
                        'MsgBox("creating import file")
    ...
    Private Sub Review_2010()
        ...
        Dim SerialNum(oXLSheet.UsedRange.Rows.Count - 1) As String
        ...
        For i = 1 To oXLSheet.UsedRange.Rows.Count - 1
            SerialNum(i) = oXLSheet.Cells(i + 1, 9).value ' Serial Number
        ...
        Array.Sort(SerialNum)
        UniqueCount = 0
        For i = 1 To UBound(SerialNum)
            If Not SerialNum(i) = SerialNum(i - 1) Then ' first record read will compare to a null value and increment counter by 1
                'If Not Int(GPSID(i)) Then
            ...
            End If
        Next
        If uniquecount < UBound(SerialNum) Then ' if we counted less uniques then there are gps id's then there must be a dupe in there somewhere
            dlgReview_2010.lblDupeCheck.ForeColor = Color.Red
            dlgReview_2010.lblDupeCheck.Left = 12
            dlgReview_2010.lblDupeCheck.Font = New Font("Microsoft Sans Serif", 11, FontStyle.Bold)
            dlgReview_2010.lblDupeCheck.Text = "Duplicate serial numbers found"
        Else
            dlgReview_2010.lblDupeCheck.ForeColor = Color.Lime
            dlgReview_2010.lblDupeCheck.Left = 25
            dlgReview_2010.lblDupeCheck.Font = New Font("Microsoft Sans Serif", 9, FontStyle.Bold)
            dlgReview_2010.lblDupeCheck.Text = "Duplicate serial number check... OK!"
        End If
        ...
        dlgReview_2010.lblRecordsFound.Text = UBound(SerialNum) & " Records found"
    ...
    Private Sub CreateAdditionalFiles()
        ...
        If DoNavImportFile = True Then
            ...
            oXLSheet.Range("BM:BY").Delete() ' Erase after the sim number to the next to last column
